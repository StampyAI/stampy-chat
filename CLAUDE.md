...
- `session 156` 2026-04-02: observed added, observed jj creates divergent changes when a commit is merged into main via PR -- the original change ID exists both locally and in main's history. `jj rebase -s children -d main` then `jj abandon --ignore-immutable "divergent/N::"` cleans it up. `jj op restore` is the escape hatch when abandon/rebase goes wrong. Key: use `jj op log` to find the last good state before attempting fixes. (jj-divergent-changes-from-merged-prs).
  ttl: 1
- `session ?` 2026-04-04: observed Tool-based RAG refactor (2b57e49) added `from anthropic.lib.tools._beta_functions import BetaFunctionTool` in tools.py but Pipfile had `anthropic = "*"`. Prod venv had 0.52.0 (too old), and `pip install -r requirements.txt` with unpinned `anthropic` won't upgrade an already-installed package. Both backend and MCP crash-looped for 800+ restarts with ModuleNotFoundError., When a refactor adds imports from newer SDK features, pin the minimum version in Pipfile immediately. `pip install pkg` is a no-op if any version is already installed; only a version constraint like `>=0.75.0` forces upgrade. Check Pipfile as part of refactor review. (deploy-pip-version-pins).
  ttl: 4
