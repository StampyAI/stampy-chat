...
class Limiter:
    ...
    def check(self, ip, session, query, now) -> str | None:
        """Return a refusal reason, or None to allow. Counts the request only if allowed."""
        if len(query) > self.max_chars: return "too_long"
        with self.lock:
            day = now - 86400
            _prune(self.all, day); _prune(self.hits[ip], day, key=lambda x: x[0])
            _prune(self.repeats[ip, query], now - 3600)
            if len(self.all) >= self.global_daily: return "global_daily"
            ...
            self.all.append(now); self.hits[ip].append((now, self.cost(query))); self.repeats[ip, query].append(now)
            return None
...
MESSAGES = {
    ...
    "ip_daily": "Stampy's daily budget for this connection is used up. Please come back tomorrow.",
