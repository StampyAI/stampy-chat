...
@mcp.tool
def search_alignment_research(query: str, k: int = 20) -> str:
    """
    ...
    """
    logger.info(f"MCP search request: query='{query}' k={k}")

    # Clamp k to reasonable bounds
    k = max(1, min(50, k))

    try:
        blocks = get_top_k_blocks(query, k)
...
@mcp.tool
def search_alignment_research_raw(query: str, k: int = 20) -> list[dict]:
    """
    ...
    """
    logger.info(f"MCP raw search request: query='{query}' k={k}")
