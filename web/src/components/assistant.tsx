...
export const ShowAssistantEntry: React.FC<{ entry: AssistantEntry }> = ({
  entry,
}) => {
        ...
        response.split("\n").map((paragraph) => (
          <p key={paragraph.length}>
            ...
            {paragraph.split(in_text_citation_regex).map((text, i) => {
              if (i % 2 === 0) {
                return <GlossarySpan key={i} content={text.trim()} />;
              }
              i = parseInt(text) - 1;
              if (!citations.has(i)) return `[${text}]`;
              const citation = citations.get(i)!;
              return <ShowInTextCitation key={i} citation={citation} i={i} />;
            })}{" "}
          </p>
        ))
      }
