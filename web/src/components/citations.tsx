...
export const ProcessText: (
  text: string,
  base_count: number
) => [string, Map<string, number>] = (text, base_count) => {
  ...
  while ((match = regex.exec(response)) !== null) {
    ...
    // replace [x] with [i]
    response_copy +=
      response.slice(response_copy.length, match.index) +
      `[${cite_map.get(match[1]!)! + 1}]`;
  }
...
export const ShowCitation: React.FC<{ citation: Citation; i: number }> = ({
  citation,
  i,
}) => {
...
export const ShowInTextCitation: React.FC<{
  citation: Citation;
  i: number;
}> = ({ citation, i }) => {
  const url =
